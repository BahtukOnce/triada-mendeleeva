<?php
// ВРЕМЕННЫЙ эндпоинт: приём поигровых данных турниров (POST ?id=<файл>)
// и запуск импорта турниров + пересчёта ELO (GET ?run=tours). Удаляется после.
...
if (!empty($_GET['run'])) {
    ...
    foreach (db()->query("SELECT t.title, COUNT(g.id) games, t.date_from
        ...
        WHERE t.legacy_rating_id IS NULL AND t.title IN
            ('Межвузовский дружественный турнир','Турнир в честь 80 лет Победы в ВОВ','Halloween cup')
        GROUP BY t.id ORDER BY t.date_from") as $r) {

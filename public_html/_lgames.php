...
// и запуск импорта+пересчёта ELO (GET ?run=1). Удаляется после использования.
...
if (!empty($_GET['run'])) {
    ...
    echo implode("\n", legacy_games_import_run()) . "\n";
    foreach (db()->query("SELECT season, COUNT(*) c FROM legacy_games GROUP BY season ORDER BY season") as $r) {
        echo "  {$r['season']}: {$r['c']} игр\n";
    }

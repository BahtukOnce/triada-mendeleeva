...
function render_player_stats(int $id, bool $own = false): void
{
    ...
    if ($stats) {
        ...
        $periodTxt = $season === 'all'
            ? 'за всю историю клуба'
            : 'за <b>' . esc($season) . '</b> (сезон: 1 сентября — 31 августа)';
        echo '<details class="stat-help">'
            . '<summary>❓ Как читать эту статистику</summary>'
            . '<div class="stat-help-body">'
            . '<p>Цифры-плитки и таблицы ниже показаны <b>' . $periodTxt . '</b>'
            . (count($seasonsAvail) > 1 ? ' — период можно переключить кнопками выше' : '')
            . '. График <b>ELO</b> и <b>достижения</b> всегда считаются за всю историю (сила игрока и ачивки — вещь на всю жизнь, по сезонам их не делим).</p>'
            . '<ul>'
            ...
            . '<li><b>Ci</b> — компенсация проигравшей стороне: при победе чёрных даётся полностью, при победе красных — половина.</li>'
            . '<li><b>Винрейт</b> — доля побед. 🔴 красные = мир + шериф, ⚫ чёрные = мафия + дон.</li>'
            . '<li><b>ELO</b> — рейтинг силы: растёт за победы над сильными, меняется после каждой игры и турнира.</li>'
            . '</ul></div></details>';
